...
import { deleteOutputPolicy, getOutputPolicyList } from '@/service/api';
  ...
  if (method === 'update') {
    ...
    window.$dialog?.warning({
      ...
      onPositiveClick: async () => {
        const { error } = await deleteOutputPolicy(row.id);
        if (error) return;
        getData();
...
async function getData() {
  loading.value = true;
  const { data: res, error } = await getOutputPolicyList();
  loading.value = false;
  if (error) return;
        ...
        <template #header>
          <div class="mb-5px mt-5px">
            ...
            <NTag :bordered="false" type="success">
              {{ $t('page.firewallPolicy.defaultPolicy') }}: {{ $t('page.firewallPolicy.accept') }}
            </NTag>
          </div>
        </template>
        <NDataTable
