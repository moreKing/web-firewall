...
import { deleteOutputLimitPolicy, getOutputLimitPolicyList } from '@/service/api';
  ...
  if (method === 'update') {
    ...
    window.$dialog?.warning({
      ...
      onPositiveClick: async () => {
        const { error } = await deleteOutputLimitPolicy(row.id);
        if (error) return;
        getData();
...
async function getData() {
  loading.value = true;
  const { data: res, error } = await getOutputLimitPolicyList();
  loading.value = false;
  if (error) return;
