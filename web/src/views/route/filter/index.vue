...
import { deleteForwardPolicy, getForwardPolicyList } from '@/service/api';
  ...
  if (method === 'update') {
    ...
    window.$dialog?.warning({
      ...
      onPositiveClick: async () => {
        const { error } = await deleteForwardPolicy(row.id);
        if (error) return;
        getData();
...

  {
    ...
    render(row: any) {
      return h(
        ...
        {
          default: () => (row.policy === 'accept' ? $t('page.firewallPolicy.accept') : $t('page.firewallPolicy.reject'))
        }
      );
    }
  },
...
async function getData() {
  loading.value = true;
  const { data: res, error } = await getForwardPolicyList();
  loading.value = false;
  if (error) return;
