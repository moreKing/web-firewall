...
import { deleteSnatPolicy, getSnatPolicyList } from '@/service/api';
  ...
  if (method === 'update') {
    ...
    window.$dialog?.warning({
      ...
      onPositiveClick: async () => {
        const { error } = await deleteSnatPolicy(row.id);
        if (error) return;
        getData();
...
async function getData() {
  loading.value = true;
  const { data: res, error } = await getSnatPolicyList();
  loading.value = false;
  if (error) return;
...
function filterDataFn() {
  ...
  filterData.value = data.filter((item: any) => {
    if (listParams.value.sourceIp && listParams.value.sourceIp.trim() !== '' && item.sip && item.sip.trim() !== '') {
      // 判断ip 是否有效，有效才进行过滤
    ...
    if (item.dip && item.dip.trim() !== '' && listParams.value.destIp && listParams.value.destIp.trim() !== '') {
      // 判断ip 是否有效，有效才进行过滤
    ...
    if (listParams.value.network && listParams.value.network.trim() !== '') {
      if (item.oif.trim() !== listParams.value.network.trim()) return false;
    }
